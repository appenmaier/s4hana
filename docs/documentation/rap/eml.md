...
Die _Entity Manpipulation Lanaguage_ (EML) ermgöglicht den lesenden und schreibenden Zugriff auf RAP BOs. Für den lesenden Zugriff stellt EML die Anweisung `READ ENTITIES`, für den schreibenden Zurgiff die Anweisung `MODIFY ENTITITES` mit den Zusatzwörtern `CREATE`, `CREATE_BY`, `UPDATE`, `DELETE` und `EXECUTE` zur Verfügung.
...
DATA keys        TYPE TABLE FOR READ IMPORT ZR_ConnectionTP.
DATA connections TYPE TABLE FOR READ RESULT ZR_ConnectionTP.
DATA reported    TYPE RESPONSE FOR REPORTED ZR_ConnectionTP.
DATA failed      TYPE RESPONSE FOR FAILED ZR_ConnectionTP.

keys = VALUE #( ( CarrierId = 'LH' ConnectionId '0400' ) ).

READ ENTITIES OF ZR_ConnectionTP
     ENTITY Connection
     ALL FIELDS
     WITH keys
     RESULT connections
...
```abap showLineNumbers
DATA connections_for_create TYPE TABLE FOR CREATE ZR_ConnectionTP.
DATA connections_for_update TYPE TABLE FOR UPDATE ZR_ConnectionTP.
DATA connections_for_delete TYPE TABLE FOR DELETE ZR_ConnectionTP.
DATA flights_for_create     TYPE TABLE FOR CREATE ZR_ConnectionTP\_Flights.
DATA connections_for_action TYPE TABLE FOR ACTION IMPORT ZR_ConnectionTP~ConvertDistance.
DATA mapped                 TYPE RESPONSE FOR MAPPED ZR_ConnectionTP.
DATA reported               TYPE RESPONSE FOR REPORTED ZR_ConnectionTP.
DATA failed                 TYPE RESPONSE FOR FAILED ZR_ConnectionTP.
...
MODIFY ENTITIES OF ZR_ConnectionTP
       ENTITY Connection
       CREATE
